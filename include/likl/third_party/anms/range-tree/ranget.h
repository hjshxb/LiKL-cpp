...
/* The following block ships the data away and uses an offset to access it.
   ...
   If you value speed over memory use, and are on a 64-bit platform, comment it
   out.
*/
...
class rangetree {
   public:
    rangetree(u32 estimatedTotal = 1000, u32 estimatedResult = 100)
        : nodepool(NULL),
          ptypool(NULL),
          mainreserve(estimatedTotal),
          resultreserve(estimatedResult),
          init(false) {
    ...
    void finalize() {
        ...
        const point maxrange =
            (xtmparray[totalsize - 1].x - xtmparray[0].x) + 1;
    ...
    std::vector<data*>* search(point xmin,
                               point xmax,
                               point ymin,
                               point ymax) const {
        if (!init) return NULL;
   ...
   private:
    ...
    u32 binarynext(const pty* const arr,
                   const u32 ycount,
    ...
    void findnodes(const node* const n,
                   const point xmin,
                   const point xmax,
                   const node* list[],
                   u32& listcur) const {
    ...
    void mergekids(pty*& arr,
                   u32& ycount,
                   node* const __restrict__ left,
...
#ifdef LR_VISUALIZE
    void visualize(const node* const n,
                   graph_t* const g,
                   Agnode_t* const n1) const {
        Agnode_t* temp;
        Agedge_t* e;
