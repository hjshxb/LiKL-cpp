...
#define TRTCHECK(status)                                                                                                  \
    ...
    {                                                                                                                  \
        ...
        if (ret != 0)                                                                                                  \
        {                                                                                                              \
            tensorrt_log::gLogError << "Cuda failure: " << ret << std::endl;                                                 \
            abort();                                                                                                   \
        }                                                                                                              \
    } while (0)

#define TRTCHECK_RETURN_W_MSG(status, val, errMsg)                                                                        \
    do                                                                                                                 \
    {                                                                                                                  \
        if (!(status))                                                                                                 \
        {                                                                                                              \
...
#define TRTCHECK_RETURN(status, val) TRTCHECK_RETURN_W_MSG(status, val, "")
...
OBJ_GUARD(T)
makeObjGuard(T_ *t) {
    TRTCHECK(!(std::is_base_of<T, T_>::value || std::is_same<T, T_>::value));
...
// Swaps endianness of an integral type.
    ...
    inline void *safeCudaMalloc(size_t memSize) {
        void *deviceMem;
        TRTCHECK(cudaMalloc(&deviceMem, memSize));
...
//! Return vector of indices that puts magnitudes of sequence in descending order.
    ...
    std::vector<std::string> classify(
        ...
        const auto inds = tensorrt_common::argMagnitudeSort(output.cbegin(), output.cend());
...
// Returns indices of highest K magnitudes in v.
    ...
    std::vector<size_t> topKMagnitudes(const std::vector<T> &v, const size_t k) {
        std::vector<size_t> indices = tensorrt_common::argMagnitudeSort(v.cbegin(), v.cend());
        indices.resize(k);
        return indices;
    }
...
// New vPPM(variable sized PPM) class with variable dimensions.
    ...
    template<int C, int H, int W>
    void readPPMFile(const std::string &filename, tensorrt_common::PPM<C, H, W> &ppm) {
    ...
    public:
        GpuTimer(cudaStream_t stream)
                : mStream(stream) {
            TRTCHECK(cudaEventCreate(&mStart));
            TRTCHECK(cudaEventCreate(&mStop));
        }

        ~GpuTimer() {
            TRTCHECK(cudaEventDestroy(mStart));
            TRTCHECK(cudaEventDestroy(mStop));
        }

        void start() {
            TRTCHECK(cudaEventRecord(mStart, mStream));
        }

        void stop() {
            TRTCHECK(cudaEventRecord(mStop, mStream));
            float ms{0.0f};
            TRTCHECK(cudaEventSynchronize(mStop));
            TRTCHECK(cudaEventElapsedTime(&ms, mStart, mStop));
            mMs += ms;
        }
...
// Return m rounded up to nearest multiple of n
...
#else
            tensorrt_log::gLogError << "Could not load plugin library: " << path << ", due to: " << dlerror()
                                    << std::endl;
#endif
        }
    }
    ...
    inline int32_t getSMVersion() {
        ...
        TRTCHECK(cudaGetDevice(&deviceIndex));

        int32_t major, minor;
        TRTCHECK(cudaDeviceGetAttribute(&major, cudaDevAttrComputeCapabilityMajor, deviceIndex));
        TRTCHECK(cudaDeviceGetAttribute(&minor, cudaDevAttrComputeCapabilityMinor, deviceIndex));
